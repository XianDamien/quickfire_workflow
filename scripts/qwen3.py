...
def evaluate_pronunciation(asr_data, qb_data, api_key=None, model="qwen-plus"):
    """
    核心评测函数：基于ASR转写和题库数据进行发音评测

    Args:
        asr_data (str): ASR转写结果（文本格式）
        qb_data (str): 题库数据（CSV格式）
        api_key (str, optional): DashScope API密钥，默认从环境变量读取
        model (str, optional): 使用的LLM模型，默认 "qwen-plus"
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": [{"text": qb_prompt}],
        },
        {
            "role": "user",
            "content": [{"text": asr_prompt}],
        },
    ]
...
if __name__ == "__main__":
    ...
    # 调用评测函数
    result = evaluate_pronunciation(asr_data, qb)
