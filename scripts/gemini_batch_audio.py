...
用法:
    python3 scripts/gemini_batch_audio.py run --archive-batch <name>

对比基准:
    python3 scripts/gemini_batch.py run --archive-batch <name>  # ASR 版本
"""
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    run_id = new_run_id() + "_audio"  # 添加后缀区分
    print(f"  Run ID: {run_id}")

    # 准备输出目录
    batch_path = archive_batch_dir(archive_batch)
    run_dir = batch_path / "_batch_runs" / run_id
    ...
    for result in results:
        ...
        try:
            ...
            fail_count += 1
            student_results.append({"student": student_name, "status": "error", "error": f"extract: {e}"})
            continue
        ...
        if parsed.get("_parse_error"):
            ...
            fail_count += 1
            student_results.append({"student": student_name, "status": "error", "error": "json_parse"})
            continue
        ...
        if not is_valid:
            ...
            fail_count += 1
            student_results.append({"student": student_name, "status": "error", "error": f"validation: {len(invalid_items)} invalid items"})
            continue
        ...
        # 保存到 run 目录
        try:
            annotator_name = f"{model}_audio"  # 区分音频版
            ...
            fail_count += 1
            student_results.append({"student": student_name, "status": "error", "error": f"save: {e}"})
...
def _generate_batch_report(
    ...
    - students/: 每个学生的完整报告（含 annotations 和错误详情）
    """
    ...
    for result in results:
        ...
        try:
            ...
            if candidates:
                ...
                if parts_list:
                    ...
                    error_detail = {"type": "no_content_parts"}
            else:
                # 检查是否有错误信息
                if "error" in result:
                    error_detail = {"type": "api_error", "message": result["error"]}
                else:
                    error_detail = {"type": "no_candidates"}
        except Exception as e:
            error_detail = {"type": "extract_exception", "message": str(e)}
        ...
        student_full_data[student_name] = {
            "annotations": annotations,
            "raw_text": raw_text[:2000] if raw_text else None,  # 保存原始文本预览
            "token_usage": token_usage,
            "grade": student_result.get("grade"),
            "mistake_count": student_result.get("mistake_count"),
            "status": student_result.get("status", "unknown"),
    ...
    for detail in student_details:
        ...
        if detail["status"] == "error":
            ...
            student_report["error_detail"] = full_data.get("error_detail")
            student_report["raw_text_preview"] = full_data.get("raw_text")
