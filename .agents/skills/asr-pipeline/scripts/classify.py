...
def build_messages(student_name: str, segs: Dict, stage_info: dict | None = None) -> List[Dict]:
    """构建单个学生的分类请求消息。"""
    lines = [f"以下是 {student_name} 的课堂片段转录：\n"]
    stage_text = format_class_stage_context(stage_info)
    if stage_text:
        lines.append(
            f"【班级阶段参考】{stage_text}。这只是弱先验，只在转写内容模糊时参考，"
            "如果与转写直接冲突，以转写内容为准。\n"
        )
...
def write_student_result(
    class_dir: Path, class_name: str, student_name: str, segs: Dict,
...
def collect_class_data(class_dir: Path, student_filter=None) -> Dict[str, Dict]:
    ...
    for student_dir in sorted(
        ...
        if not metadata:
            continue
        segments = load_segments(raw_meta) or {
            f"{seg}/2_qwen_asr.json": {"type": truth, "qb_file": None}
        ...
        student_segs = {}
        for seg, truth in metadata.items():
            asr_dir = seg_dir_map.get(seg)
            asr_path = None
            if asr_dir:
                for suffix in ("2_qwen_asr.txt", "2_qwen_asr.json"):
                    candidate = asr_dir / suffix
                    if candidate.exists():
                        asr_path = candidate
                        break
        ...
        student_data[student_dir.name] = {
            "segments": student_segs,
...
def run_classify(args) -> int:
    ...
    for class_dir in class_dirs:
        ...
        for student_name, student_payload in student_data.items():
            segs = student_payload["segments"]
            stage_info = student_payload.get("stage_info")
            ...
            messages = build_messages(student_name, segs, stage_info=stage_info)
            if use_gemini:
            ...
            t, c = write_student_result(
                class_dir, class_name, student_name, segs, predictions, args.model,
            )
            grand_total += t
            grand_correct += c
